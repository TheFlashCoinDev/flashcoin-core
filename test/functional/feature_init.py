#!/usr/bin/env python3
# Copyright (c) 2021-present The FlashCoin developers
...
class InitTest(FlashCoinTestFramework):
    """
    ...
    def init_stress_test(self):
        ...
        def sigterm_node():
            if platform.system() == 'Windows':
                ...
                # FlashCoind to perform any shutdown logic.
    ...
    def init_pid_test(self):
        FlashCoin_PID_FILENAME_CUSTOM = "my_fancy_FlashCoin_pid_file.foobar"

        self.log.info("Test specifying custom pid file via -pid command line option")
        custom_pidfile_relative = FlashCoin_PID_FILENAME_CUSTOM
        ...
        assert not (datadir / FlashCoin_PID_FILENAME_DEFAULT).exists()
        assert (datadir / custom_pidfile_relative).exists()
        self.stop_node(0)
        assert not (datadir / custom_pidfile_relative).exists()

        custom_pidfile_absolute = Path(self.options.tmpdir) / FlashCoin_PID_FILENAME_CUSTOM
