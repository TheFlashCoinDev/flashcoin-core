...
# Copyright 2016-present The FlashCoin developers
...
class ToolUtils(FlashCoinTestFramework):
    ...
    def skip_test_if_missing_module(self):
        self.skip_if_no_FlashCoin_tx()
        self.skip_if_no_FlashCoin_util()

    def run_test(self):
        self.testcase_dir = Path(self.config["environment"]["SRCDIR"]) / "test" / "functional" / "data" / "util"
        self.bins = self.get_binaries()
        with open(self.testcase_dir / "FlashCoin-util-test.json", encoding="utf8") as f:
    ...
    def test_one(self, testObj):
        ...
        if testObj["exec"] == "./FlashCoin-util":
            execrun = self.bins.util_argv() + testObj["args"]
        elif testObj["exec"] == "./FlashCoin-tx":
