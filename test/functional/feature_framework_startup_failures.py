#!/usr/bin/env python3
# Copyright (c) 2025-present The FlashCoin developers
...
without the parent process' FlashCoinTestFramework also failing.
"""
...
class FeatureFrameworkStartupFailures(FlashCoinTestFramework):
    ...
    def _verify_startup_failure(self, test, internal_args, expected_exception):
        ...
        try:
            ...
            print("Unexpected child process timeout!\n"
                  ...
                  "meaning dangling FlashCoind processes are to be expected.\n"
    ...
    def run_test(self):
        ...
        self.log.info("Verifying inability to connect to FlashCoind's RPC interface due to wrong port results in one exception containing at least one OSError.")
        self._verify_startup_failure(
            TestWrongRpcPortStartupFailure, [f"--internal_node_start_duration={node_start_duration}"],
            r"AssertionError: \[node 0\] Unable to connect to FlashCoind after \d+s \(ignored errors: {[^}]*'OSError \w+'?: \d+[^}]*}, latest: '[\w ]+'/\w+\([^)]+\)\)"
        )

        self.log.info("Verifying startup failure due to invalid arg results in only one exception.")
        self._verify_startup_failure(
            TestInitErrorStartupFailure, [],
            r"FailedToStartError: \[node 0\] FlashCoind exited with status 1 during initialization\. Error: Error parsing command line arguments: Invalid parameter -nonexistentarg"
        )
...
class TestWrongRpcPortStartupFailure(InternalTestMixin, FlashCoinTestFramework):
    def add_options(self, parser):
...
class TestInitErrorStartupFailure(InternalTestMixin, FlashCoinTestFramework):
    def set_test_params(self):
        self.num_nodes = 1
...
class TestStartStopStartupFailure(InternalTestMixin, FlashCoinTestFramework):
    def set_test_params(self):
        self.num_nodes = 1
...
class TestSuccess(InternalTestMixin, FlashCoinTestFramework):
    def set_test_params(self):
        self.num_nodes = 1
