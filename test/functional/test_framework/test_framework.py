#!/usr/bin/env python3
# Copyright (c) 2014-present The FlashCoin developers
...
TMPDIR_PREFIX = "FlashCoin_func_test_"
...
class Binaries:
    """Helper class to provide information about FlashCoin binaries
    ...
    def node_argv(self, **kwargs):
        "Return argv array that should be used to invoke FlashCoind"
        return self._argv("node", self.paths.FlashCoind, **kwargs)

    def rpc_argv(self):
        "Return argv array that should be used to invoke FlashCoin-cli"
        # Add -nonamed because "FlashCoin rpc" enables -named by default, but FlashCoin-cli doesn't
        return self._argv("rpc", self.paths.FlashCoincli) + ["-nonamed"]

    def tx_argv(self):
        "Return argv array that should be used to invoke FlashCoin-tx"
        return self._argv("tx", self.paths.FlashCointx)

    def util_argv(self):
        "Return argv array that should be used to invoke FlashCoin-util"
        return self._argv("util", self.paths.FlashCoinutil)

    def wallet_argv(self):
        "Return argv array that should be used to invoke FlashCoin-wallet"
        return self._argv("wallet", self.paths.FlashCoinwallet)

    def chainstate_argv(self):
        "Return argv array that should be used to invoke FlashCoin-chainstate"
        return self._argv("chainstate", self.paths.FlashCoinchainstate)

    def _argv(self, command, bin_path, need_ipc=False):
        """Return argv array that should be used to invoke the command. It
        either uses the FlashCoin wrapper executable (if FlashCoin_CMD is set or
        need_ipc is True), or the direct binary path (FlashCoind, etc). When
        bin_dir is set (by tests calling binaries from previous releases) it
        always uses the direct path."""
        if self.bin_dir is not None:
            return [os.path.join(self.bin_dir, os.path.basename(bin_path))]
        elif self.paths.FlashCoin_cmd is not None or need_ipc:
            # If the current test needs IPC functionality, use the FlashCoin
            # wrapper binary and append -m so it calls multiprocess binaries.
            FlashCoin_cmd = self.paths.FlashCoin_cmd or [self.paths.FlashCoin_bin]
            return FlashCoin_cmd + (["-m"] if need_ipc else []) + [command]
        else:
            return [bin_path]


class FlashCoinTestMetaClass(type):
    """Metaclass for FlashCoinTestFramework.

    Ensures that any attempt to register a subclass of `FlashCoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'FlashCoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("FlashCoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("FlashCoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class FlashCoinTestFramework(metaclass=FlashCoinTestMetaClass):
    """Base class for a FlashCoin test script.

    Individual FlashCoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use FlashCoin-cli instead of RPC for all commands")
    ...
    def get_binary_paths(self):
        ...
        binaries = {
            "FlashCoin": "FlashCoin_BIN",
            "FlashCoind": "FlashCoinD",
            "FlashCoin-cli": "FlashCoinCLI",
            "FlashCoin-util": "FlashCoinUTIL",
            "FlashCoin-tx": "FlashCoinTX",
            "FlashCoin-chainstate": "FlashCoinCHAINSTATE",
            "FlashCoin-wallet": "FlashCoinWALLET",
        }
        # Set paths to FlashCoin binaries allowing overrides with environment
        # variables.
        ...
        # FlashCoin_CMD environment variable can be specified to invoke FlashCoin
        # wrapper binary instead of other executables.
        paths.FlashCoin_cmd = shlex.split(os.getenv("FlashCoin_CMD", "")) or None
        return paths
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("FlashCoinRPC")
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a FlashCoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple FlashCoind test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the FlashCoind
    ...
    def _start_logging(self):
        ...
        # Format logs the same as FlashCoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in FlashCoin.conf
    ...
    def skip_if_no_FlashCoind_tracepoints(self):
        """Skip the running test if FlashCoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("FlashCoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_FlashCoind_zmq(self):
        """Skip the running test if FlashCoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("FlashCoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if FlashCoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("FlashCoin-wallet has not been compiled")

    def skip_if_no_FlashCoin_tx(self):
        """Skip the running test if FlashCoin-tx has not been compiled."""
        if not self.is_FlashCoin_tx_compiled():
            raise SkipTest("FlashCoin-tx has not been compiled")

    def skip_if_no_FlashCoin_util(self):
        """Skip the running test if FlashCoin-util has not been compiled."""
        if not self.is_FlashCoin_util_compiled():
            raise SkipTest("FlashCoin-util has not been compiled")

    def skip_if_no_FlashCoin_chainstate(self):
        """Skip the running test if FlashCoin-chainstate has not been compiled."""
        if not self.is_FlashCoin_chainstate_compiled():
            raise SkipTest("FlashCoin-chainstate has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if FlashCoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("FlashCoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether FlashCoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether FlashCoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_FlashCoin_tx_compiled(self):
        """Checks whether FlashCoin-tx was compiled."""
        return self.config["components"].getboolean("BUILD_FlashCoin_TX")

    def is_FlashCoin_util_compiled(self):
        """Checks whether FlashCoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_FlashCoin_UTIL")

    def is_FlashCoin_chainstate_compiled(self):
        """Checks whether FlashCoin-chainstate was compiled."""
        return self.config["components"].getboolean("ENABLE_FlashCoin_CHAINSTATE")
