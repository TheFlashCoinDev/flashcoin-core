...
def wait_until_helper_internal(predicate, *, timeout=60, lock=None, timeout_factor=1.0, check_interval=0.05):
    ...
    from `FlashCoinTestFramework` or `P2PInterface` class ensures the timeout is
...
def bpf_cflags():
    return [
        ...
        "-Wno-duplicate-decl-specifier",  # https://github.com/FlashCoin/FlashCoin/issues/32322
    ]
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "FlashCoin.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> tuple[dict, pathlib.Path]:
    ...
    if platform.system() == "Windows":
        ...
        datadir = temp_dir / "FlashCoin"
    else:
        env = dict(HOME=str(temp_dir))
        if platform.system() == "Darwin":
            datadir = temp_dir / "Library/Application Support/FlashCoin"
        else:
            datadir = temp_dir / ".FlashCoin"
    return env, datadir
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "FlashCoin.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "FlashCoin.conf")):
        with open(os.path.join(datadir, "FlashCoin.conf"), 'r', encoding='utf8') as f:
            for line in f:
