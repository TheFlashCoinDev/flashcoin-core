This directory contains integration tests that test FlashCoind and its
...
FlashCoind and FlashCoin-qt by interacting with them through the RPC and P2P
interfaces.
...
Before tests can be run locally, FlashCoin must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the FlashCoind nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another FlashCoind
process running on the system (perhaps from a previous test which hasn't successfully
killed all its FlashCoind nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other FlashCoind processes are running.

On linux, the test framework will warn if there is another
FlashCoind process running when the tests are started.

If there are zombie FlashCoind processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
FlashCoind processes running on the system, so should not be used if any non-test
FlashCoind processes are being run.**
...
sure FlashCoind processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and FlashCoind `debug.log`s will all be dumped
...
`test_framework.log` and FlashCoind `debug.log`s can be combined into a single
...
call methods that interact with the FlashCoind nodes-under-test.

If further introspection of the FlashCoind instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/FlashCoind.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/FlashCoind <pid>
```
