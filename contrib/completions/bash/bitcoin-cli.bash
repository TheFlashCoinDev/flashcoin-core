# bash programmable completion for FlashCoin-cli(1)
# Copyright (c) 2012-2022 The FlashCoin developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $FlashCoin-cli for RPC
_FlashCoin_rpc() {
...
_FlashCoin_cli() {
    ...
    # save and use original argument to invoke FlashCoin-cli for -help, help and RPC
    # as FlashCoin-cli might not be in $PATH
    FlashCoin_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($FlashCoin_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_FlashCoin_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
...
complete -F _FlashCoin_cli FlashCoin-cli
