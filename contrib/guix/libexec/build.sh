#!/usr/bin/env bash
# Copyright (c) 2019-2022 The FlashCoin developers
...
(
    ...
    case "$HOST" in
        *mingw*)
            ...
            mv build/FlashCoin-win64-setup.exe "${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our FlashCoin build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built FlashCoin to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    (
        ...
        # copy over the example FlashCoin.conf file. if contrib/devtools/gen-FlashCoin-conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/FlashCoin.conf" "${DISTNAME}/"
    ...
    case "$HOST" in
        ...
        *darwin*)
            ...
            mv build/dist/FlashCoin-Core.zip "${OUTDIR}/${DISTNAME}-${HOST}-unsigned.zip"
