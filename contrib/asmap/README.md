...
be used in FlashCoin with the `-asmap` option.
...
to binary before being used with FlashCoin.
...
the `FlashCoin-cli getnodeaddresses` command.
...
```
FlashCoin-cli getnodeaddresses 0 > addrs.json
```
