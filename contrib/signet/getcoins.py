#!/usr/bin/env python3
# Copyright (c) 2020-2022 The FlashCoin developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing FlashCoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='FlashCoin-cli', help='FlashCoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='FlashCoin address to which the faucet should send')
...
parser.add_argument('FlashCoin_cli_args', nargs='*', help='Arguments to pass on to FlashCoin-cli (default: -signet)')

args = parser.parse_args()

if args.FlashCoin_cli_args == []:
    args.FlashCoin_cli_args = ['-signet']


def FlashCoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.FlashCoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = FlashCoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = FlashCoin_cli(['getnewaddress', 'faucet', 'bech32'])
