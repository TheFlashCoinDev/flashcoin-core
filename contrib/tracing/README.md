...
Tracing (USDT) support for FlashCoin on Linux using. For more information on
USDT support in FlashCoin see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `FlashCoind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `FlashCoind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[FlashCoin USDT documentation] on how to list available tracepoints in your
`FlashCoind` binary.

[FlashCoin USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof FlashCoind)
```
...
FlashCoin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof FlashCoind)
```
...
In a different terminal, starting FlashCoin in SigNet mode and with
re-indexing enabled.

```
$ ./build/bin/FlashCoind -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof FlashCoind)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof FlashCoind)
```
