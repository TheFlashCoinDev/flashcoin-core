...
Provides utilities for managing copyright headers of `The FlashCoin
...
---------------------------------------------------------
Updates all the copyright headers of `The FlashCoin developers` which were
changed in a year more recent than is listed. For example:
```
// Copyright (c) <firstYear>-<lastYear> The FlashCoin developers
```
will be updated to:
```
// Copyright (c) <firstYear>-<lastModifiedYear> The FlashCoin developers
```
...
those cases:
```
// Copyright (c) <year> The FlashCoin developers
```
will be updated to:
```
// Copyright (c) <year>-<lastModifiedYear> The FlashCoin developers
```
where the update is appropriate.

copyright\_header.py insert \<file\>
------------------------------------
Inserts a copyright header for `The FlashCoin developers` at the top of the
...
If the file already has a copyright for `The FlashCoin developers`, the
script will exit.
...
gen-FlashCoin-conf.sh
===================

Generates a FlashCoin.conf file in `share/examples/` by parsing the output from `FlashCoind --help`. This script is run during the
release process to include a FlashCoin.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-FlashCoin-conf.sh
```
