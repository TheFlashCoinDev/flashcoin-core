...
"""Script for verifying FlashCoin release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from FlashCoincore.org and FlashCoin.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/FlashCoin-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://FlashCoincore.org"
HOST2 = "https://FlashCoin.org"
VERSIONPREFIX = "FlashCoin-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the FlashCoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"FlashCoin_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by FlashCoincore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the FlashCoin release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes FlashCoin.org lags behind FlashCoincore.org.)')
    )
