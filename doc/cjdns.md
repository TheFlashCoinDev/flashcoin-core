# CJDNS support in FlashCoin

It is possible to run FlashCoin over CJDNS, an encrypted IPv6 network that
...
done outside of FlashCoin, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the FlashCoin network and individual nodes.
...
before it will work with your FlashCoin node. This is described in steps
...
## Run FlashCoin with CJDNS

Once you are connected to the CJDNS network, the following FlashCoin
...
When enabled, this option tells FlashCoin that it is running in an
...
IPv6 local network. This helps FlashCoin perform better address management:
...
CJDNS support was added to FlashCoin in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `FlashCoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/FlashCoin/FlashCoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in FlashCoin

There are several ways to see your CJDNS address in FlashCoin:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `FlashCoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `FlashCoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `FlashCoin-cli help getnodeaddresses` for details.
