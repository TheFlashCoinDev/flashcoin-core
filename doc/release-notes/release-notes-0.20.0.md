...
FlashCoin version 0.20.0 is now available from:

  <https://FlashCoincore.org/bin/FlashCoin-core-0.20.0/>
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  FlashCoin
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use FlashCoin on
unsupported systems.

From FlashCoin 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, FlashCoin does not yet change appearance
...
- Instead of running `make` simply, you should instead run
  `FlashCoin_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from FlashCoin

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since FlashCoin version 0.18.0.
...
- Testing or debugging of implementations of the FlashCoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of FlashCoin. FlashCoin logs debug messages
...
- OpenSSL is no longer used by FlashCoin.  (#17265)

- BIP70 support has been fully removed from FlashCoin. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/FlashCoin/FlashCoin/issues/16599),
  [PR #16702](https://github.com/FlashCoin/FlashCoin/pull/16702), and the
  `FlashCoind help` for more information.  This option is experimental and
...
- All custom settings configured when FlashCoin starts are now
...
- The "Start FlashCoin on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://FlashCoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple FlashCoin wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed FlashCoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/FlashCoin-core/HWI) for
  signing.  Future versions of FlashCoin should support a GUI option
...
- FlashCoin's automatically-generated source code documentation is
  now available at https://doxygen.FlashCoincore.org.  (#17596)
...
- The `FlashCoin-cli` utility used with the `-getinfo` parameter now
...
- The `FlashCoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `FlashCoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_FlashCoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBFlashCoin_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_FlashCoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBFlashCoin_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for FlashCoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate FlashCoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest FlashCoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_FlashCoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (FlashCoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone FlashCoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about FlashCoin files (hebasto)
...
- #17119 Fix broken FlashCoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from FlashCoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.FlashCoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to FlashCoin-wallet (hel-o)
...
- #16115 On FlashCoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from FlashCoin-cli -getinfo (malevolent)
