FlashCoin version 29.0 is now available from:

  <https://FlashCoincore.org/bin/FlashCoin-core-29.0/>
...
previously FlashCoin would listen for incoming Tor connections on `127.0.0.1:8334`.
...
127.0.0.1:5556`, or configure FlashCoind with `-bind=127.0.0.1:8334=onion` to get the previous behavior.
(#31223)
...
- As a safety check, FlashCoin will **fail to start** when `-blockreservedweight` init parameter value is lower than `2000` weight units. FlashCoin will also **fail to start** if the `-blockmaxweight` or `-blockreservedweight` init parameter exceeds consensus limit of `4,000,000 WU`.
...
3. CMake variables may be used to configure the build system. **Some defaults have changed.** For example, you will now need to add `-DWITH_ZMQ=ON` to build with zmq and `-DBUILD_GUI=ON` to build `FlashCoin-qt`. See [Autotools to CMake Options Mapping](https://github.com/FlashCoin-core/FlashCoin-devwiki/wiki/Autotools-to-CMake-Options-Mapping) for details.
4. For single-configuration generators, the default build configuration (`CMAKE_BUILD_TYPE`) is "RelWithDebInfo". However, for the "Release" configuration, CMake defaults to the compiler optimization flag `-O3`, which has not been extensively tested with FlashCoin. Therefore, the build system replaces it with `-O2`.
...
```
cmake -B build
cmake --build build --target FlashCoind
cmake --install build --component FlashCoind
```
...
- A new tool [`utxo_to_sqlite.py`](https://github.com/FlashCoin/FlashCoin/blob/v29.0/contrib/utxo-tools/utxo_to_sqlite.py)
