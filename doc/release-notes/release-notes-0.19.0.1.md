FlashCoin version 0.19.0.1 is now available from:

  <https://FlashCoincore.org/bin/FlashCoin-core-0.19.0.1/>
...
FlashCoin is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use FlashCoin on unsupported systems.
...
to open the FlashCoin .dmg. This is due to new signing requirements
imposed by Apple, which the FlashCoin project does not yet adhere too.
...
- [Reduce memory](https://github.com/FlashCoin/FlashCoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running FlashCoin on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of FlashCoin
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `FlashCoind -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile FlashCoin with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in FlashCoin 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of FlashCoin. (#14054)

- To eliminate well-known denial-of-service vectors in FlashCoin,
  ...
  This prevents FlashCoin from sending the BIP111 NODE_BLOOM service
  ...
  of FlashCoin and nodes that have manually enabled BIP37 support,
...
- By default, FlashCoin will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `FlashCoin-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how FlashCoin determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  FlashCoin would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, FlashCoin will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 FlashCoin address without
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in FlashCoin v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@FlashCoin AEAD (jonasschnelli)
...
- #15639 FlashCoin-wallet tool: Drop libFlashCoin_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in FlashCoind/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "FlashCoin-binaries/version" directory (hebasto)
...
- #16441 remove qt libjpeg check from FlashCoin_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_FlashCoin to auto generated project files (sipsorcery)
...
- #15773 Add FlashCoinTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_FlashCoin output (practicalswift)
...
- #15896 feature_filelock, interface_FlashCoin_cli: Use PACKAGE_NAME in messages rather than hardcoding FlashCoin (luke-jr)
...
- #16277 Suppress output in test_FlashCoin for expected errors (gertjaap)
...
- #16489 log: harmonize FlashCoind logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (FlashCoinVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example FlashCoin.conf (fanquake)
...
- #15683 Comment for seemingly duplicate LIBFlashCoin_SERVER (Bushstar)
...
- #16080 build/doc: update FlashCoin_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in FlashCoin.conf (fanquake)
...
- #16621 add default FlashCoin.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the FlashCoin-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, FlashCoin.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/FlashCoind.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/FlashCoin/FlashCoin/).
