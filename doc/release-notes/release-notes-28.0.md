FlashCoin version 28.0 is now available from:

  <https://FlashCoincore.org/bin/FlashCoin-core-28.0/>
...
Running FlashCoin binaries on macOS requires self signing.
```
cd /path/to/FlashCoin-28.0/bin
xattr -d com.apple.quarantine FlashCoin-cli FlashCoin-qt FlashCoin-tx FlashCoin-util FlashCoin-wallet FlashCoind test_FlashCoin
codesign -s - FlashCoin-cli FlashCoin-qt FlashCoin-tx FlashCoin-util FlashCoin-wallet FlashCoind test_FlashCoin
```
...
using the Linux Kernel 3.17+, macOS 11.0+, and Windows 7 and newer. FlashCoin
...
Support for Testnet4 as specified in [BIP94](https://github.com/FlashCoin/bips/blob/master/bip-0094.mediawiki)
...
The default data directory on Windows has been moved from `C:\Users\Username\AppData\Roaming\FlashCoin`
to `C:\Users\Username\AppData\Local\FlashCoin`. FlashCoin will check the existence
...
See [JSON-RPC-interface.md](https://github.com/FlashCoin/FlashCoin/blob/master/doc/JSON-RPC-interface.md#json-rpc-11-vs-20) for details. (#27101)
...
libFlashCoinconsensus Removal
---------------------------

The libFlashCoin-consensus library was deprecated in 27.0 and is now completely removed. (#29648)

P2P and Network Changes
-----------------------

- Previously if FlashCoin was listening for P2P connections, either using
...
- FlashCoin will now fail to start up if any of its P2P binds fail, rather
...
- Transactions with version number set to 3 are now treated as standard on all networks (#29496),
  ...
  described in [BIP 431](https://github.com/FlashCoin/bips/blob/master/bip-0431.mediawiki).  The
...
- The `warnings` field in `getblockchaininfo`, `getmininginfo` and
  ...
  can be temporarily restored by running FlashCoin with the configuration
...
are now required to compile FlashCoin. (#29091, #30263)

- The minimum required glibc to run FlashCoin is now
...
- A new `-testdatadir` option has been added to `test_FlashCoin` to allow specifying the
...
Previous releases of FlashCoin or previous external software will not be able to read the blocksdir with a non-zero XOR-key.
