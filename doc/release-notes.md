...
FlashCoin version v30.0 is now available from:

  <https://FlashCoincore.org/bin/FlashCoin-core-30.0/>
...
installer (on Windows) or just copy over `/Applications/FlashCoin-Qt` (on macOS)
...
FlashCoin is supported and tested on operating systems using the
Linux Kernel 3.17+, macOS 13+, and Windows 10+. FlashCoin
...
New `FlashCoin` command
---------------------

- A new `FlashCoin` command line tool has been added to make features more discoverable
  and convenient to use. The `FlashCoin` tool just calls other executables and does not
  implement any functionality on its own. Specifically `FlashCoin node` is a synonym for
  `FlashCoind`, `FlashCoin gui` is a synonym for `FlashCoin-qt`, and `FlashCoin rpc` is a synonym
  for `FlashCoin-cli -named`. Other commands and options can be listed with `FlashCoin help`.
  The new `FlashCoin` command is an alternative to calling other commands directly, but it
...
- The new `FlashCoin` command does support one new feature: an (experimental) IPC Mining
  Interface that allows the node to work with Stratum v2 or other mining client software,
  see (#31098). When the node is started with `FlashCoin -m node -ipcbind=unix` it will
  ...
  (`FlashCoin-node` instead of `FlashCoind`) and is currently required but will become optional
  in the future (with [#33229](https://github.com/FlashCoin/FlashCoin/pull/33229)).

- IPC connectivity introduces new dependencies (see [multiprocess.md](https://github.com/FlashCoin/FlashCoin/blob/master/doc/multiprocess.md)),
...
- The `test_FlashCoin` executable is now installed in `libexec/` instead of `bin/`.
  It can still be executed directly, or accessed through the new `FlashCoin` command
  as `FlashCoin test`. The `libexec/` directory also contains new `FlashCoin-node` and
  `FlashCoin-gui` binaries which support IPC features and are called through the
  `FlashCoin` tool. In source builds only, `test_FlashCoin-qt`, `bench_FlashCoin`, and
  `FlashCoin-chainstate` are also now installed to `libexec/` instead of `bin/` and
  can be accessed through the new `FlashCoin` command. See `FlashCoin help` output for
  details. (#31679)
...
- The implementation of coinstatsindex was changed to prevent an overflow bug that
  ...
  from their datadir. A future release of FlashCoin may remove the old version of the
...
- The `-paytxfee` startup option and the `settxfee` RPC are now deprecated and
  will be removed in FlashCoin 31.0. They allowed the user to set a static fee
...
- The legacy wallet removal drops redundant options in the FlashCoin-wallet tool,
...
[Transifex](https://explore.transifex.com/FlashCoin/FlashCoin/).
