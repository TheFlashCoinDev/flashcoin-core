# User-space, Statically Defined Tracing (USDT) for FlashCoin

FlashCoin includes statically defined tracepoints to allow for more
...
**Note**: FlashCoin uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to FlashCoin
...
Multiple tools can list the available tracepoints in a `FlashCoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in FlashCoin, use `info probes` in `gdb`:

```
$ gdb ./build/bin/FlashCoind
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e 0x0000000000d29bd2 /build/bin/FlashCoind
stap net        outbound_message 0x0000000000107c05 0x0000000000d29bd0 /build/bin/FlashCoind
stap validation block_connected  0x00000000002fb10c 0x0000000000d29bd8 /build/bin/FlashCoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in FlashCoin.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./build/bin/FlashCoind | grep NT_STAPSDT -A 4 -B 2
...
```
$ tplist -l ./build/bin/FlashCoind -v
