...
Below are some notes on how to build FlashCoin for Windows.

The options known to work for building FlashCoin on Windows are:
...
Note that for WSL the FlashCoin source path MUST be somewhere in the default mount file system, for
example /usr/src/FlashCoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\FlashCoin`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/FlashCoin
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/FlashCoin --strip
```
