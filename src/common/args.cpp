...
const char * const FlashCoin_CONF_FILENAME = "FlashCoin.conf";
const char * const FlashCoin_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //FlashCoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", FlashCoin_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\FlashCoin
    //   new: C:\Users\Username\AppData\Local\FlashCoin
    // macOS: ~/Library/Application Support/FlashCoin
    // Unix-like: ~/.FlashCoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "flashcoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "flashcoin";
#else
    fs::path pathRet;
