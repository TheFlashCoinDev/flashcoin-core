// Copyright (c) 2011-2022 The FlashCoin Core developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <FlashCoin-build-config.h> // IWYU pragma: keep

#include <qt/FlashCoin.h>
...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<FlashCoinUnit>("FlashCoinUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. FlashCoin_de.qm (shortcut "de" needs to be defined in FlashCoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. FlashCoin_de_DE.qm (shortcut "de_DE" needs to be defined in FlashCoin.qrc)
...
static const char* qt_argv = "FlashCoin-qt";

FlashCoinApplication::FlashCoinApplication()
...
void FlashCoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the FlashCoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", FlashCoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void FlashCoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool FlashCoinApplication::createOptionsModel(bool resetSettings)
{
...
void FlashCoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new FlashCoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &FlashCoinGUI::quitRequested, this, &FlashCoinApplication::requestShutdown);
...
void FlashCoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void FlashCoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool FlashCoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void FlashCoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &FlashCoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &FlashCoinApplication::handleRunawayException);
    connect(this, &FlashCoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &FlashCoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void FlashCoinApplication::parameterSetup()
{
...
void FlashCoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void FlashCoinApplication::requestInitialize()
{
...
void FlashCoinApplication::requestShutdown()
{
...
void FlashCoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // FlashCoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &FlashCoinGUI::handlePaymentRequest);
            connect(window, &FlashCoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void FlashCoinApplication::handleRunawayException(const QString &message)
{
...
void FlashCoinApplication::handleNonFatalException(const QString& message)
{
...
WId FlashCoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool FlashCoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", FlashCoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(FlashCoin);
    Q_INIT_RESOURCE(FlashCoin_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(FlashCoin_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see FlashCoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see FlashCoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse FlashCoin.conf, determine network, switch to network specific
    ...
    // FlashCoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
