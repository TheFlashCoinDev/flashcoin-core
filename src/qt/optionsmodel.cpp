// Copyright (c) 2011-present The FlashCoin developers
...
/** Call node.updateRwSetting() with FlashCoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because FlashCoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/FlashCoin/FlashCoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because FlashCoin 22.x will not try to read these.
...
//! Convert enabled/size values to FlashCoin -prune setting.
...
//! Get pruning enabled value to show in GUI from FlashCoin -prune setting.
...
//! Get pruning size value to show in GUI from FlashCoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayFlashCoinUnit")) {
        settings.setValue("DisplayFlashCoinUnit", QVariant::fromValue(FlashCoinUnit::BTC));
    }
    QVariant unit = settings.value("DisplayFlashCoinUnit");
    if (unit.canConvert<FlashCoinUnit>()) {
        m_display_FlashCoin_unit = unit.value<FlashCoinUnit>();
    } else {
        m_display_FlashCoin_unit = FlashCoinUnit::BTC;
        settings.setValue("DisplayFlashCoinUnit", QVariant::fromValue(m_display_FlashCoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if FlashCoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_FlashCoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<FlashCoinUnit>() == m_display_FlashCoin_unit) return;
    m_display_FlashCoin_unit = new_unit.value<FlashCoinUnit>();
    QSettings settings;
    settings.setValue("DisplayFlashCoinUnit", QVariant::fromValue(m_display_FlashCoin_unit));
    Q_EMIT displayUnitChanged(m_display_FlashCoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/FlashCoin/FlashCoin/pull/8273
    ...
    // (https://github.com/FlashCoin-core/gui/issues/567).
