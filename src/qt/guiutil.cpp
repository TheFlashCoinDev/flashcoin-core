...
#include <qt/FlashCoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setValidator(new FlashCoinAddressEntryValidator(parent));
    widget->setCheckValidator(new FlashCoinAddressCheckValidator(parent));
}
...
bool parseFlashCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no FlashCoin: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!FlashCoinUnits::parse(FlashCoinUnit::BTC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseFlashCoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseFlashCoinURI(uriInstance, out);
}

QString formatFlashCoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("FlashCoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(FlashCoinUnits::format(FlashCoinUnit::BTC, info.amount, false, FlashCoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openFlashCoinConf()
{
    ...
    /* Open FlashCoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("FlashCoin (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for FlashCoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / fs::u8path(strprintf("FlashCoin-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a FlashCoin.desktop file to the autostart directory:
