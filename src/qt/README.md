This directory contains the source code for the FlashCoin graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the FlashCoin UI.
...
- ClientModel is used by the main application `FlashCoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a FlashCoin:... link from a browser).
...
* UI elements like FlashCoinAmountField, which inherit from QWidget.
* `FlashCoinstrings.cpp`: automatically generated
* `FlashCoinunits.(h/cpp)`: BTC / mBTC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/FlashCoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `FlashCoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `FlashCoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `build/bin`, and select `FlashCoin-qt`
9. You're all set! Start developing, building, and debugging the FlashCoin GUI
