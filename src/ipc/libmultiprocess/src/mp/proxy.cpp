...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking FlashCoin code so they have to run
    ...
    // functions run, so client objects are fully disconnected before FlashCoin
    // code in the destructors are run. This way if the FlashCoin code tries to
...
void EventLoop::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In FlashCoin, running these callbacks in the right order is
