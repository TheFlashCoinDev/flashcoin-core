...
static void SetupFlashCoinTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " FlashCoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The FlashCoin-tx tool is used for creating and modifying FlashCoin transactions.\n\n"
                "FlashCoin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded FlashCoin transaction, or with \"-create [commands]\" to create a hex-encoded FlashCoin transaction.\n"
                "\n"
                "Usage: FlashCoin-tx [options] <hex-tx> [commands]\n"
                "or:    FlashCoin-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded FlashCoin transaction
