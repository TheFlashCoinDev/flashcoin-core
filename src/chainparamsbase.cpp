...
{
    // Selección de cadena: main / testnet / testnet4 / signet / regtest
    argsman.AddArg(
        "-chain=<chain>",
    ...
    // Altura de activación custom para pruebas (solo regtest)
    argsman.AddArg(
    ...
    argsman.AddArg(
        "-testnet",
        "Use the testnet3 chain. Equivalent to -chain=test. Support for testnet3 is deprecated and will be removed in an upcoming release. "
    ...
    // Parámetros de version bits (solo regtest)
    argsman.AddArg(
    ...
    argsman.AddArg(
        "-signetseednode",
        "Specify a seed node for the signet network, in the hostname[:port] format, e.g. sig.net:1234 "
        "(may be used multiple times to specify multiple seed nodes; defaults to the global default signet test network seed node(s))",
        ArgsManager::ALLOW_ANY | ArgsManager::DISALLOW_NEGATION,
        OptionsCategory::CHAINPARAMS);
}
...
std::unique_ptr<CBaseChainParams> CreateBaseChainParams(const ChainType chain)
{
    switch (chain) {
    case ChainType::MAIN:
        return std::make_unique<CBaseChainParams>("", 17554);
    case ChainType::TESTNET:
        return std::make_unique<CBaseChainParams>("testnet3", 27554);
    case ChainType::TESTNET4:
        return std::make_unique<CBaseChainParams>("testnet4", 37554);
    case ChainType::SIGNET:
        return std::make_unique<CBaseChainParams>("signet", 47554);
    case ChainType::REGTEST:
        return std::make_unique<CBaseChainParams>("regtest", 57554);
    }
    assert(false);
}
